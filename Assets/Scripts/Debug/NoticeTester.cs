...
{
    /// <summary>
    /// 레벨업·스킬 해금 알림(<see cref="LevelUpNotice"/> · <see cref="SkillUnlockBanner"/>)을
    /// 플레이 모드에서 눈으로 확인하는 테스트 하네스.
    /// 씬 아무 GameObject에 붙이고 플레이 → 인스펙터 컨텍스트 메뉴(⋮)로 실행한다.
    /// </summary>
    /// <remarks>
    /// 이게 필요한 이유: 두 알림은 아직 <b>아무 데서도 호출되지 않는다</b>.
    /// <c>PlayerEvents.FireLevelUp</c>은 호출부가 없고 스킬 해금 시스템은 스텁이라,
    /// 실제로 플레이해서는 이 알림이 뜨는 것을 볼 방법이 없다(2026.08.12 TH).
    /// 배선이 붙으면 이 하네스는 연출 타이밍 조정용으로만 남는다.
    /// </remarks>
