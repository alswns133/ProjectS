...
{
    /// <summary>
    /// NPC 머리 위 퀘스트 표시(! / ?). 같은 NPC의 <see cref="QuestGiver"/> 상태를 읽어 아이콘을 정하고,
    /// 표시할 게 없으면 숨긴다. 상태는 매 프레임 폴링하지 않고 퀘스트/레벨 이벤트에 반응해 갱신한다
    /// (수락하면 !가 사라지고, 목표를 다 채우면 ?가 뜨는 식). 아이콘은 항상 카메라를 향하도록 빌보드한다.
    ///
    /// 배치: NPC 머리 위에 월드 스페이스 아이콘(Image)을 두고 이 컴포넌트를 붙인다.
    /// questGiver를 비워두면 부모에서 자동으로 찾는다.
    /// </summary>
    public class QuestMarker : MonoBehaviour
    {
        ...
        // 부모의 QuestGiver를 확보한다(인스펙터 미지정 시).
        private void Awake()
        {
            if (questGiver == null)
        ...
        // QuestGiver 상태에 맞춰 아이콘을 바꾸거나 숨긴다.
        private void Refresh()
        {
            if (questGiver == null || iconImage == null) return;

            switch (questGiver.EvaluateMarkerState())
            {
