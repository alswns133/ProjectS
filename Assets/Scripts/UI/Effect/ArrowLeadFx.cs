...
namespace ProjectS.UI
{
    ...
    /// 고정돼, 화살표가 어디를 가리키든 잔상은 늘 같은 화면 방향으로 뻗는다.
    /// </summary>
    ...
    public class ArrowLeadFx : MonoBehaviour
    {
        ...
        [Header("잔상")]
        [Tooltip("화살표가 가리키는 방향. 잔상은 본체에서 출발해 이쪽으로 나아간다.\n" +
        ...
        private void Apply()
        {
            ...
            {
                startPosition = (Vector2)transform.InverseTransformPoint(sourceRect.position);
                flow = (Vector2)(Quaternion.Inverse(transform.rotation) * (sourceRect.rotation * (Vector3)direction.normalized));
            }
            else
            {
                startPosition = sourceRect.anchoredPosition;
                flow = direction.normalized;
            }

            Vector2 endPosition = startPosition + flow * distance;
            float cycle = Cycle;
