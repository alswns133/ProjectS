Shader "UI/Hologram"
{
    Properties
    {
        ...
        // 글리치 (몇 초에 한 번씩 '툭' 터지는 버스트 방식)
        _GlitchInterval ("Glitch Interval (sec)", Range(0.2, 10)) = 3.0  // 버스트 사이 평균 간격
        _GlitchDuration ("Glitch Duration (sec)", Range(0.02, 1)) = 0.15 // 한 번 터질 때 지속 시간
        _GlitchSpeed    ("Glitch Tear Speed",     Range(1, 60))   = 25.0 // 버스트 중 찢어짐 갱신 속도 (빠를수록 프래틱)
        _GlitchStrength ("Glitch Strength",       Range(0, 0.1))  = 0.02
        _GlitchThreshold("Glitch Threshold",      Range(0, 1))    = 0.7  // 낮출수록 버스트 때 더 많은 행이 밀림
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                // ── 글리치: 몇 초에 한 번씩 짧게 터지는 버스트 ──
                // 시간을 _GlitchInterval 길이의 '슬롯'으로 나누고,
                // 각 슬롯 안 랜덤한 지점에서 _GlitchDuration 만큼만 발동.
                float interval   = max(_GlitchInterval, 0.01);
                float slot       = floor(_Time.y / interval);
                float slotRand   = rand(float2(slot, 91.7));            // 이 슬롯의 버스트 시작점(0~1)
                float localTime  = _Time.y - slot * interval;          // 0 ~ interval
                float burstStart = slotRand * max(interval - _GlitchDuration, 0.0);
                // 지금이 버스트 구간 안인가?
                float inBurst    = step(burstStart, localTime)
                                 * step(localTime, burstStart + _GlitchDuration);

                float glitchX = 0;
                if (inBurst > 0.5)
                {
                    // 버스트 중에는 빠르게 갱신되는 시드로 '찢어짐' 연출
