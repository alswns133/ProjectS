// ExternalAssets/SpecialSkillsEffectsPack/Shader_ForEffects/Shader_IntegratedEffect.shader 의 복사본.
// 원본과 유일한 차이는 포그를 받지 않는다는 점입니다(fog 관련 4줄 주석 처리, "NOFOG" 표시).
// 원본 팩은 .gitignore로 제외되는 경로라 팀에 공유되지 않으므로, 팩을 고치는 대신 트래킹되는 곳에 복사했습니다.
Shader "GAPH Custom Shader/Shader_IntegradedEffect_NoFog"
{
		Properties {
			[Header(Main)]
				[Space]
			...
			Category{
					...
					SubShader {
						Pass{
							...
							// NOFOG: 포그 변형(variant) 자체를 컴파일하지 않습니다.
							...
							struct v2f {
								...
								#endif
								// NOFOG: fogCoord 보간값 제거(TEXCOORD8은 비게 됩니다).
								//UNITY_FOG_COORDS(8)
								#ifdef IS_RIMLIGHT
							...
							half4 frag(v2f i): SV_Target
							{
								...
								#endif
								// NOFOG: 원본은 여기서 결과색을 검정으로 페이드시켜 포그를 적용했습니다.
