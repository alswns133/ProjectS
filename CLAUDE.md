...
- **Animation Event 슬롯 키의 언더바 표기 차이는 무시합니다 (2026-07-29 확정).** 히트박스/투사체/이펙트
  ...
  모두 허용합니다. 스킬 키는 캐릭터 이름 접두사를 답니다(`Haru_Skill3_1`, 2026-09-23 도입) — 이름은
  `PlayerStatTable.CharacterName`에서 오며 게이트에 하드코딩하지 않습니다. 접두사가 없거나 다른 키는
  게이트가 막고 경고를 남깁니다(2026-09-23 이관 완료, 폴백 제거). **단, 언더바가 아니라 토큰 자체가 다른 키(`Skill3` vs `Skill3_Wave`)는 흡수되지 않으므로
...
### ID 넘버링 규칙 (2026-07-28 확정, 프로젝트 표준)

- 모든 데이터 테이블의 ID는 **`docs/ID_NUMBERING.md`** 규칙을 따릅니다. 새 행을 만들거나 ID를 참조할 때 이 문서를 기준으로 합니다.
- 요약: 몬스터 4자리 `[던전][난이도][순번2]` · 아이템 6자리 `[분류][등급][종류][레벨/순번3]`(1무기·2방어구·3소비·4재료) · 스킬 캐릭터 3자리 `[캐릭터][스킬2]`(04=각성기) · 퀘스트 6자리 `[종류][지역2][순번3]`(5메인·6반복) · 던전/스테이지 2자리(=몬스터 앞2자리).
- 원칙: 각 테이블은 자기 안에서만 유일하면 됩니다(테이블 간 숫자 겹침 무방). ID가 값을 인코딩하고 컬럼에도 같은 값이 있으면, 그 테이블 `Validate(out error)`에서 **둘의 일치를 검사**해 오타를 로딩 시점에 거릅니다(현재 `QuestTable` 적용, 몬스터·아이템은 권장).
...
- **플레이어 강공격·스킬 시전 중 슈퍼아머 (2026-08-03 확정, 2026-08-14 강공격 포함으로 확장)**:
  플레이어는 어떤 공격을 받아도 데미지는 받지만,
  피격 경직(`PlayerHitState`)은 **플레이어가 지금 무슨 동작 중인가**로 가릅니다. **일반 공격·대시 공격·
  점프 공격 중에는 피격 시 정상적으로 경직**되고, **우클릭 강공격·캐릭터 스킬(궁 포함) 시전 중에는
  슈퍼아머가 걸려 약한 피격은 데미지만 받고 경직되지 않습니다.** 스킬·강공격을 한 번 지르면 잡몹 평타에 계속
  끊기지 않게 하려는 기획입니다. 예외로 **강피격(`PlayerStats.LastHitWasStrong` — 받은 피해가
  `strongHitThreshold` 이상)은 슈퍼아머를 뚫고 경직시킵니다.** → 강공격·스킬 시전 중 약한 피격에 경직이 안 걸리는
  것은 버그가 아니라 이 결정입니다.
  - 구현: `PlayerCombat.IsSuperArmorMove`(= `currentAction == Skill || StrongAttack`, 대시/점프는 제외)를
    노출하고, `Player.OnDamaged`에서 `Combat.IsSuperArmorMove && !Stats.LastHitWasStrong`이면
    `HitState` 진입을 건너뜁니다. 데미지 적용 자체는 그 전에 `PlayerStats.TakeDamage`가 이미 끝냈으므로
    "데미지만 받고 경직만 생략"이 됩니다.
