...
- **플레이어 스킬 시전 중 슈퍼아머 (2026-08-03 확정)**: 플레이어는 어떤 공격을 받아도 데미지는 받지만,
  피격 경직(`PlayerHitState`)은 **플레이어가 지금 무슨 동작 중인가**로 가릅니다. **일반 공격·대시 공격·
  점프 공격·우클릭 강공격 중에는 피격 시 정상적으로 경직**되고, **캐릭터 스킬(궁 포함) 시전 중에는
  슈퍼아머가 걸려 약한 피격은 데미지만 받고 경직되지 않습니다.** 스킬을 한 번 지르면 잡몹 평타에 계속
  끊기지 않게 하려는 기획입니다. 예외로 **강피격(`PlayerStats.LastHitWasStrong` — 받은 피해가
  `strongHitThreshold` 이상)은 슈퍼아머를 뚫고 경직시킵니다.** → 스킬 시전 중 약한 피격에 경직이 안 걸리는
  것은 버그가 아니라 이 결정입니다.
  - 구현: `PlayerCombat.IsCastingSkillMove`(= `currentAction == Skill`, 강공격/대시/점프는 제외)를
    노출하고, `Player.OnDamaged`에서 `Combat.IsCastingSkillMove && !Stats.LastHitWasStrong`이면
