...
- **몬스터 피격 반응(경직) 유발 조건 (2026-08-03 확정)**: 몬스터는 모든 타격에 데미지는 받지만,
  피격 경직(`HitState`)은 공격 종류로 가릅니다. **일반 공격·대시 공격·점프 공격·우클릭 강공격은 경직을
  유발하고, 스킬은 기본적으로 데미지만 주고 경직시키지 않습니다.** 스킬 연타 중 몬스터가 계속 밀려나거나
  행동이 끊기지 않게 하려는 기획입니다. 예외로 **`PlayerCombat`의 히트박스/투사체 슬롯에서 강피격(`strongHit`)으로
  지정한 스킬 타격만 경직을 유발**합니다. → 스킬이 몬스터를 경직시키지 않는 것은 버그가 아니라 이 결정입니다.
  - 구현: 타격 종류를 `HitReaction`(None/Normal/Strong) 값으로 만들어(`PlayerCombat.ResolveHitReaction`),
    `AttackContext`→`DamageResult`에 실어 보내고(근접/투사체 공통 경로), `Enemy.OnDamaged(HitReaction)`이
    `None`이면 `HitState` 진입을 건너뜁니다. 몬스터→플레이어 공격은 이 값을 안 쓰고 기본 `None`이라
    플레이어 피격 경직(별도 `Damaged` 이벤트 경로)에는 영향이 없습니다.
  - 강피격(Strong)은 현재 몬스터 피격 클립이 하나뿐이라 실효 동작이 일반 피격(Normal)과 같습니다. 값은 구분해
    실어 두었으니, 강한 경직/전용 모션을 붙이려면 `EnemyAnimation`에 클립·파라미터를 추가하고
    `Enemy.OnDamaged`/`EnemyHitState`에서 분기하면 됩니다(공격 측 코드는 손대지 않아도 됨).
