...
/// <summary>
/// Google OAuth Desktop App의 Authorization Code + PKCE 흐름을 사용한다.
/// refresh token은 현재 Windows 사용자만 읽을 수 있도록 DPAPI로 암호화해 저장한다.
/// </summary>
internal sealed class GoogleOAuthClient : IDisposable
{
    ...
    private static readonly byte[] TokenEntropy = SHA256.HashData(
        Encoding.UTF8.GetBytes("ProjectS.ExternalAssetsLauncher.GoogleOAuth.v1"));
    ...
    public async Task<string> GetAccessTokenAsync(string configurationPath, CancellationToken cancellationToken)
    {
        ...
        var token = await LoadTokenAsync(cancellationToken)
            ?? throw new InvalidOperationException("먼저 런처에서 Google 로그인 버튼을 눌러 팀원 계정으로 로그인하세요.");
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                await WriteBrowserResponseAsync(context, false, "로그인 검증에 실패했습니다. 런처로 돌아가 다시 시도하세요.");
            ...
            {
                await WriteBrowserResponseAsync(context, false, "인증 코드를 받지 못했습니다. 런처로 돌아가 다시 시도하세요.");
                throw new InvalidOperationException("Google 로그인 응답에 인증 코드가 없습니다.");
            }

            await WriteBrowserResponseAsync(context, true, "Google 로그인이 완료되었습니다. 이 창을 닫고 런처로 돌아가세요.");
    ...
    private static async Task WriteBrowserResponseAsync(HttpListenerContext context, bool succeeded, string message)
    {
        ...
        var html = $"<!doctype html><html><head><meta charset=\"utf-8\"><title>ProjectS Launcher</title></head><body style=\"font-family:Segoe UI,sans-serif;padding:32px\"><h2 style=\"color:{color}\">{encodedMessage}</h2></body></html>";
